...
3つとも持っているのはここだけ。判定の閾値は `waitcurve.py` に定数で置き、
根拠の数値も一緒に画面に出す（判定だけ出して計算を隠すと、閾値が合わないときに
読み替えられない）。同じ混雑帯の日が `MIN_DAYS`（既定3日）たまるまでは
「データ不足」と表示する。

**答えるのは「今買うか、待つか」。** 一日を通した固定の評価は出さない。理由は2つ。

1. **過ぎた時間帯は根拠にならない。** 16時に立っている人に「10時なら空いていた」は
   無意味。`advise()` は基準時刻より前の時間帯を捨てて計算する。
2. **アトラクションのDPAは60分に1つしか買えない。** 公式FAQによると、購入から60分後、
   または利用開始時刻のいずれか早い時刻を過ぎるまで次のアトラクションのDPAを買えない
   （同じ施設は利用後に再購入可。パレード／ショーは別枠）
   （[公式FAQ](https://faq.tokyodisneyresort.jp/answer/67eaa65b7605a4cb7775c099/)）。
   つまり同じ時刻に推せるのは常に1つで、それは施設を横断して比べないと決まらない。

そこで `advise_by_hour()` が開園から閉園までの各時刻について判定表を作り、画面は
現在時刻（今日以外の日は選んだ時刻）の行を引く。**「今」はブラウザにしか無いので、
判定を先に表にして渡し、ロジックをJS側へ写さない。** 横断の順位付け
（売切目安が近いものを先に取り、同条件なら1分あたりが安い順）と、下記の枠の
取り合いだけが画面側の仕事になる。

**閉園前1時間の枠は全施設で取り合いになる。** ここを根拠に「買わなくていい」と
言えるのは、その帯で最短の1施設だけ。8施設が揃って「20時台に並べば空く」と出ても、
20時台に乗れるのは1つしかない。残りは「枠の取り合い」として別に出す。
...
- **空と失敗を区別する。** 翌月未掲載は `note: "not_published"`、取得失敗は `note: "fetch_failed"`。
  UIの表示も別にしてある。
- **「取得していない日」を「取れていない」と表示しない。** 公式サイトは今日・明日・来園日の
  3日しか取らないので、それ以外の日は `official` が無い。これを取得失敗と同じ扱いにすると、
  収集が成功している日でも鮮度警告が鳴り、変更差分が「変更なし」と嘘をつく。
  鮮度の「公式サイト」行は日付側ではなく `health.official.last_success_at`（収集そのもの）を見る。
- **パーク単位の取得失敗を「無い」と表示しない。** `parks[park] = {"note": "fetch_failed"}` の
  ときは、ショー欄が「掲載がありません」、休止欄が「休止はありません」と出ると嘘になる
  （当日限りの休止が丸ごと落ちるため）。どちらも取得できていない旨に差し替えてある。
...
- **混雑度は「予想」と「実績」が同じ欄に入っている。** Queue-Times はその日を過ぎると
  予想値を実績値で上書きする。実測では 2026-09-06 の TDS が **予想62% → 実績16%** と
  48ポイント動き、帯を2つまたいだ（未来日の予想も日々1〜2ポイント動く）。
  来園日には実績が存在しないので、蓄積側を実績・来園日を予想で突き合わせると
  **別の物差しを比べる**ことになる。`crowd_calendar.keep_forecast()` が未来日は最新の予想を
  追い、日を過ぎたら「その朝に見えていた予想」で凍結する。帯の判定は `band_pct()` 経由で
  常に `forecast_pct` を使う。実績は `crowd_pct` に残るので、予想の当たり外れは後から見られる。
- **Queue-Times の「0分」は待ち0とは限らない。** 列を締め切ったあとも `is_open` は true のまま
  `wait_time` が 0 になる。実測（2026-09-05 TDS）でソアリンは 19:55 に 80分 → 20:00 に 0分 へ
  1ステップで落ち、`is_open` が false になったのは閉園後の 21:05 だった。この 0 を集計に入れると
  「案内終了の時刻」がその施設の一番空く時間帯として採用され、DPA判定が全施設「買わなくてよい」に
  倒れる。`waitcurve.drop_after_queue_close()` が、まとまった待ちから 0 に落ちて 0 が続いた区間を
  施設ごとに落とす。朝一番の本物の 0（直前に有効値が無い）と、もともと待ちが短い施設の 0 は残す。
